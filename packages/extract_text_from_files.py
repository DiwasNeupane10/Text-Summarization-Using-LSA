...
def extract_text(file):
    extension = file.filename.rsplit(".", 1)[
        1
    ].lower()  # splits the file name from the right after encountering the first '.'.[1].lower means converting the extension to lowercase
    flag = True
    text = []
    ...
    match extension:
        case "pdf":
            try:
                ...
                filename = (
                    secure_filename(
                        file.filename.rsplit(".", 1)[0] + "_" + current_date_time
                    )
                    + ".txt"
                )  # after spliting the filename takes the filename and adds extension of'.txt'
                path = os.path.join(
                    "./extraction", filename
                )  # path for the extracted test to be save at
                ...
                with open(path, "r", encoding="utf-8") as f:
                    content = (
                        f.read().strip()
                    )  # reads the file and strips the whitespaces from it
                    if not bool(
                        content
                    ):  # bool(content) return true if there is text in the content
