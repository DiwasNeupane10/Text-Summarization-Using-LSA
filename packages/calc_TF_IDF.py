...
def compute_total_words_in_sentence(no_of_sentences,preprocessed_sentences):
    ...
    for i in range(no_of_sentences):#iterate over the sentences
        sentence_total_words_dict[i]=count_words(preprocessed_sentences[i])#at the index(sentence_no) the total words in that sentence is added.
...
def count_word_appear_in_sentence(no_of_sentences,preprocessed_sentences):
    ...
    for i in range(no_of_sentences):#iterates over the total no of sentences
        word_count={}#dictionary to store word occurance without specifiying the sentence no
        words=word_split(preprocessed_sentences[i])#total wordset
        for word in words:#iterate over the words
            if word in word_count:#checks if the word is alreaddy in the dictionary as a key
                word_count[word]+=1
            else:
                word_count[word]=1
        word_occurance_dict[i]=word_count#addd the dictionary in the key(index) ie sentence no
...
def compute_tf(dict1,dict2):
    tf_dict={}
    for sentence_no ,word_occurance in dict2.items():#iterates over the key value pairs of the word_occurance_dict
        tf_dict[sentence_no]={}#initializes a nested dictionary
        total_words=dict1[sentence_no]#gets the total word count from sentence_total_words dict
...
def compute_isf(no_of_sentences,preprocessed_sentences):
    ...
    for i in range(no_of_sentences):
        words=set(word_split(preprocessed_sentences[i]))#converting to set inorder to not have repeated words
        for word in words:
